...
public class MapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void showLocation(Refugee ref) {
        ...
        if(ref.changed.equals("false")){
            imageBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.refugeeicon);
            resizedBitmap = Bitmap.createScaledBitmap(imageBitmap, 60, 60, false);
            refugee = mMap.addMarker(new MarkerOptions().position(location).title(ref.firstName).snippet(ref.firstName + "^" + ref.lastName + "^"+ ref.phone + "^"+ ref.email + "^" + ref.marital + "^" + ref.children).icon(BitmapDescriptorFactory.fromBitmap(resizedBitmap)));
        }
        else {
